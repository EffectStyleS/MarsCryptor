...
public class MarsCryptor
{
    ...
    private readonly uint[] _key = new uint[14];
    
    private static uint LeftRotation(uint u, int count) => (u << count) | (u >> (32 - count));
    private static uint RightRotation(uint u, int count) => (u >> count) | (u << (32 - count));
    private static int MathMod(int a, int b) => (Math.Abs(a * b) + a) % b;
    ...
    {
        const int tmpLength = 15;
        var tmp = new uint[tmpLength];
        Array.Copy(Key, tmp, Key.Length);
        tmp[Key.Length] = (uint)Key.Length;

        for (var j = 0; j < 4; j++)
        {
            for (var i = 0; i < tmpLength; i++)
            {
                tmp[i] = tmp[i] ^ LeftRotation(tmp[MathMod(i - 7, tmpLength)] ^ tmp[MathMod(i - 2, tmpLength)], 3) ^
                         (uint)(4 * i + j);
            }
        ...
        for (var i = BlockSize; i < 40 - BlockSize; i += 2)
        {
            _expandedKey[i] = FixSubkey(_expandedKey[i], _expandedKey[i + 3]);
        }
    }

    private static uint FixSubkey(uint k, uint r)
    {
        uint[] b = { 0xa4a8d57b, 0x5b5d193b, 0xc8a8309b, 0x73f9a978 };
        var j = k & 3;
        k |= 3;

        var m1 = ~k ^ (k << 1);     // если соседние биты совпадают, ставится 1
        var m2 = m1 & (m1 << 1);    // сокращаем последовательности 1 на 1
        m2 &= m2 << 2;                  // еще на 2
        m2 &= m2 << 4;                  // еще на 4
    ...
    private static void EnDirectMixing(IList<uint> workData, int number)
    {
        //S0=S[:256]; S1=S[256:]
        ...
        (workData[3], workData[2], workData[1], workData[0]) = (workData[0], workData[3], workData[2], workData[1]);
    }
    
    private void EnCryptotransformation(IList<uint> workData, int i)
    {
    ...
    private static void EnReverseMixing(IList<uint> workData, int number)
    {
        switch (number)
        {
            case 2:
    ...
    private static void DeDirectMixing(IList<uint> workData, int i)
    {
        (workData[3], workData[2], workData[1], workData[0]) = (workData[2], workData[1], workData[0], workData[3]);
    ...
    private void DeCryptotransformation(IList<uint> workData, int i)
    {
        (workData[3], workData[2], workData[1], workData[0]) = (workData[2], workData[1], workData[0], workData[3]);
    ...
    private static void DeReverseMixing(IList<uint> workData, int i)
    {
        (workData[3], workData[2], workData[1], workData[0]) = (workData[2], workData[1], workData[0], workData[3]);
